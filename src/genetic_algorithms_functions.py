...
def calculate_fitness(route,
    ...
    for idx in range(len(route) -  1):
        first_node, second_node = (route[idx], route[idx + 1])
        distance = distance_matrix[first_node][second_node]
...
def select_in_tournament(population,
    ...
    for tournament_iteration in range(number_tournaments):
        items = np.random.choice(list(range(len(population))), tournament_size)
        best_idx = items[np.argmax(scores[items])]
